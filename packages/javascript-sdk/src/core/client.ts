...
export class UsermavenClient {
  ...
  private initializeBrowserFeatures(): void {
    ...
    if (
      ...
      AutoCapture.enabledForProject(this.config.key) &&
      !this.config.disableAutocaptureListenerRegistration
    ) {
    ...
    if (this.config.formTracking) {
      const trackingType =
        this.config.formTracking === true ? 'all' : this.config.formTracking;
      this.formTracking = FormTracking.getInstance(
        this,
        trackingType || 'none',
        {
  ...
  private createEventPayload(
    ...
    if (eventName === '$autocapture') {
      const autocaptureAttributes = this.processAutocaptureAttributes(
        eventProps || {},
      );
