...
class AutoCapture {
        ...
        _each(eventElements, (eventElement) => {
          if (
            ...
            props[customProperty['name']] =
              this.extractCustomPropertyValue(customProperty);
          }
        });
      });
    });
  ...
  private extractCustomPropertyValue(
    customProperty: AutoCaptureCustomProperty,
  ): string {
    ...
    _each(
      ...
      function (matchedElem) {
        ...
        if (
          ['input', 'select'].indexOf(matchedElem.tagName.toLowerCase()) > -1
        ) {
  ...
  public static enabledForProject(
    token: string,
    numBuckets: number = 10,
    numEnabledBuckets: number = 10,
  ): boolean {
    if (!token) {
      return false;
    }
