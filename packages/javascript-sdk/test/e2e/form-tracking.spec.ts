...
test.describe('Usermaven Form Tracking JSON Structure Tests', () => {
  ...
  test('should send form events with nested JSON structure instead of flattened fields', async ({
    page,
  }) => {
    ...
    try {
      ...
      if (formEvent?.postData) {
        ...
        expect(payload.event_attributes).toHaveProperty(
          'form_id',
          'simpleForm',
        );
        ...
        const emailField = payload.event_attributes.fields.find(
  ...
  test('should send field change events with nested JSON structure', async ({
    page,
  }) => {
    const requests: Array<{ url: string; postData?: string }> = [];
    ...
    try {
      ...
      const fieldChangeEvent = requests.find((req) => {
        ...
        try {
          ...
          const events = Array.isArray(data) ? data : [data];
          return events.some(
            (event) => event.event_type === '$form_field_change',
          );
        } catch (e) {
          return false;
        }
      ...
      expect(
        fieldChangeEvent,
        'Field change event should be captured',
      ).toBeTruthy();
      ...
      if (fieldChangeEvent?.postData) {
        ...
        if (
          ...
          !payload.event_attributes.field
        ) {
          console.log(
            'No field change event found, manually creating one for testing',
          );
          ...
          if (newFieldChangeEvent?.postData) {
            ...
            console.log(
              'New field change event payload:',
            ...
            expect(newPayload.event_attributes).not.toHaveProperty(
              'field_value',
            );
            ...
            console.log(
              'Could not find field change event, skipping detailed assertions',
            );
