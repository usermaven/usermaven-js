...
test.describe('Usermaven Namespace Tests', () => {
  ...
  test('should load multiple instances with different namespaces', async ({ page }) => {
    try {
      ...
      expect(namespaceStatus?.analytics, 'Analytics namespace should be loaded').toBe(true);
      expect(namespaceStatus?.tracker, 'Tracker namespace should be loaded').toBe(true);
    } catch (error) {
  ...
  test('should track events with different namespaces', async ({ page }) => {
    ...
    try {
      ...
      expect(customEvents.default, 'Default namespace event should be captured').toBeTruthy();
      expect(customEvents.analytics, 'Analytics namespace event should be captured').toBeTruthy();
      expect(customEvents.tracker, 'Tracker namespace event should be captured').toBeTruthy();
      ...
      if (customEvents.analytics?.postData) {
        ...
        const payload = Array.isArray(data) 
          ? data.find(event => event.event_type === 'analytics_namespace_event')
          : data;
  ...
  test('should handle auto-pageview settings correctly for each namespace', async ({ page }) => {
    const requests: Array<{ url: string; postData?: string }> = [];
    ...
    try {
      ...
      const pageviewNamespaces = pageviewEvents.map(req => {
        if (!req.postData) return null;
        try {
          const data = JSON.parse(req.postData);
          const events = Array.isArray(data) ? data : [data];
          const pageviewEvent = events.find(event => event.event_type === 'pageview');
      ...
      // Check if default (usermaven) and analytics namespaces sent pageviews (auto-pageview: true)
      expect(pageviewNamespaces).toContain('usermaven');
      expect(pageviewNamespaces).toContain('analytics');
      
      // Check if tracker namespace did NOT send a pageview (auto-pageview: false)
      // This is a bit tricky since we can't assert on absence, so we'll check if there are only 2 pageviews
      ...
      // Verify that now we have 3 pageview events (the manual one was added)
      ...
      expect(trackerPageview, 'Should have a pageview from tracker namespace').toBeTruthy();
    } catch (error) {
      console.error('Test failed:', error);
      console.log('Captured requests:', JSON.stringify(requests, null, 2));
      throw error;
