...
test.describe('Usermaven Non-AMD/jQuery Integration Tests', () => {
  ...
  test('should load jQuery correctly', async ({ page }) => {
    ...
    const jqueryWorks = await page.isVisible('#test1-results .success:has-text("DOM manipulation works")');
  ...
  test('should load Usermaven via script tag correctly', async ({ page }) => {
    ...
    await page.waitForSelector('#test2-results .success:has-text("Script loaded successfully")', { timeout: 10000 });
    
    // Wait a bit for the script to fully initialize
    await page.waitForTimeout(2000);
    
    // Check if the success message for usermavenClient is present
    const successMessages = await page.$$eval('#test2-results .success', (elements) => {
    ...
    const clientAvailable = successMessages.some(msg => msg && msg.includes('usermavenClient global'));
    expect(clientAvailable, 'usermavenClient global should be found').toBeTruthy();
    
    // Verify event was tracked
    const eventTracked = successMessages.some(msg => msg && msg.includes('Event sent via global'));
    ...
    const scriptTagEvent = findEventInRequests(requests, 'test_event_script_load');
  ...
  test('should track events via global client', async ({ page }) => {
    ...
    const successMessages = await page.$$eval('#test3-results .success', (elements) => {
      return elements.map(el => el.textContent);
    });
    ...
    // Verify event was tracked
    const eventTracked = successMessages.some(msg => msg && msg.includes('Event tracked using global client'));
    expect(eventTracked, 'Event should be tracked using global client').toBeTruthy();
  ...
  test('should track events via jQuery event handlers', async ({ page }) => {
    ...
    const handlerMessages = await page.$$eval('#test4-results .success', (elements) => {
      return elements.map(el => el.textContent);
    });
    ...
    const handlersSetup = handlerMessages.some(msg => msg && msg.includes('jQuery event handler bound'));
    ...
    const clickMessages = await page.$$eval('#test4-results .success', (elements) => {
      return elements.map(el => el.textContent);
    });
    ...
    const clickTracked = clickMessages.some(msg => msg && msg.includes('Tracked button click'));
  ...
  test('should run all tests sequentially', async ({ page }) => {
    ...
    const allSuccessMessages = await page.$$eval('.success', (elements) => {
      return elements.map(el => ({ 
        testId: el.closest('.test-section')?.querySelector('h2')?.textContent || 'unknown',
    ...
    const jqueryWorks = allSuccessMessages.some(msg => 
      msg.testId.includes('Test 1') && msg.message.includes('DOM manipulation works'));
    expect(jqueryWorks, 'jQuery DOM manipulation should work').toBeTruthy();
    
    // Verify script tag test passed
    const scriptLoaded = allSuccessMessages.some(msg => 
      msg.testId.includes('Test 2') && msg.message.includes('Script loaded successfully'));
    expect(scriptLoaded, 'Script should load successfully').toBeTruthy();
    
    // Verify event tracking test passed
    const eventTracked = allSuccessMessages.some(msg => 
      msg.testId.includes('Test 3') && msg.message.includes('Event tracked using global client'));
    expect(eventTracked, 'Event should be tracked using global client').toBeTruthy();
    
    // Verify jQuery event handler test passed
    const handlersSetup = allSuccessMessages.some(msg => 
      msg.testId.includes('Test 4') && msg.message.includes('jQuery event handler bound'));
    expect(handlersSetup, 'jQuery event handlers should be bound').toBeTruthy();
    
    // Verify that events were sent to the server
    expect(requests.length, 'Should have captured network requests').toBeGreaterThan(0);
