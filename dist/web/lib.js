...
function getTrackingHost(scriptSrc) {
    return scriptSrc.replace("/s/" + jsFileName, "").replace("/" + jsFileName, "");
}
var supressInterceptionWarnings = "data-suppress-interception-warning";
function hookWarnMsg(hookType) {
    return "\n      ATTENTION! ".concat(hookType, "-hook set to true along with defer/async attribute! If ").concat(hookType, " code is inserted right after Usermaven tag,\n      first tracking call might not be intercepted! Consider one of the following:\n       - Inject usermaven tracking code without defer/async attribute\n       - If you're sure that events won't be sent to ").concat(hookType, " before Usermaven is fully initialized, set ").concat(supressInterceptionWarnings, "=\"true\"\n       script attribute\n    ");
}
function getTracker(window) {
    ...
    usermavenProps.forEach(function (prop) {
        var attr = "data-" + prop.replace("_", "-");
    ...
    var usermaven = function () {
        var queue = window.usermavenQ = window.usermavenQ || [];
...
function processQueue(queue, usermavenInstance) {
    getLogger().debug("Processing queue", queue);
    for (var i = 0; i < queue.length; i += 1) {
        var _a = (__spreadArray([], queue[i], true) || []), methodName = _a[0], args = _a.slice(1);
        var method = usermavenInstance[methodName];
...
if (window) {
    var win_1 = window;
    var tracker_1 = getTracker(win_1);
    if (tracker_1) {
        getLogger().debug("Usermaven in-browser tracker has been initialized");
        win_1.usermaven = function () {
            var queue = win_1.usermavenQ = win_1.usermavenQ || [];
            queue.push(arguments);
            processQueue(queue, tracker_1);
        };
        if (win_1.usermavenQ) {
            getLogger().debug("Initial queue size of ".concat(win_1.usermavenQ.length, " will be processed"));
            processQueue(win_1.usermavenQ, tracker_1);
        }
    }
    else {
